...
import { orderItems } from '../data/products';
...
export default function OrderForm() {
  ...
  const [quantities, setQuantities] = useState<Quantities>(
    Object.fromEntries(orderItems.map((item) => [item.id, 0]))
  );
  ...
  const total = orderItems.reduce(
    (acc, item) => acc + (quantities[item.id] || 0) * item.price,
    0
  );
      ...
      <div className="container mx-auto px-5 md:px-10 max-w-[1200px]">
        <div className="text-center max-w-[600px] mx-auto mb-16 animate-on-scroll">
          ...
          <p className="text-base leading-[1.7]" style={{ color: '#8B7A6B' }}>
            Sélectionnez vos pains, indiquez vos coordonnées, et nous préparons tout pour le jour
            de retrait choisi.
          </p>
        </div>
        ...
        <div className="max-w-[800px] mx-auto">
          {submitted && (
            ...
            >
              ...
              <button
                ...
                onClick={() => {
                  setSubmitted(false);
                  setQuantities(Object.fromEntries(orderItems.map((item) => [item.id, 0])));
                }}
          ...
          >
            ...
            <div>
              <strong>Important :</strong> Toute commande doit être passée au minimum{' '}
              <strong>2 jours à l'avance</strong>. Nous ne cuisons que ce qui a été commandé —
          ...
          <form
            ...
            style={{
              background: 'white',
              boxShadow: '0 4px 16px rgba(45,31,20,0.08)',
            }}
          >
            <div
            ...
            >
              ...
              <button
                type="submit"
                className="inline-flex items-center gap-2.5 px-9 py-4 rounded-full text-sm font-semibold uppercase tracking-widest text-white transition-all duration-300 hover:-translate-y-0.5"
                style={{
                  background: '#A67C52',
